...
const validationSchema = Yup.object().shape({
    // User (Admin)
    user_email: Yup.string().email(errorMessages.invalidFormat).required(errorMessages.required),
    user_fullName: Yup.string().required(errorMessages.required),
    user_password: Yup.string()
        .required(errorMessages.required)
        .min(4, `${errorMessages.invalidFormat}, at least 4 characters`)
        .max(8, `${errorMessages.invalidFormat}, at most 8 characters`),
    user_confirmPassword: Yup.string()
        .oneOf([Yup.ref("user_password")], "Passwords must match, please check!")
        .required(errorMessages.required),

    // Company
    company_name: Yup.string().required(errorMessages.required),
    company_email: Yup.string().email(errorMessages.invalidFormat),
    company_vatId: Yup.string().matches(vatRegex, errorMessages.invalidFormat).required(errorMessages.required),
    company_phoneNumber1: Yup.string().matches(phoneRegExp, `${errorMessages.invalidFormat} (ex. +34 123456456)`),
    company_phoneNumber2: Yup.string().matches(phoneRegExp, `${errorMessages.invalidFormat} (ex. +34 123456456)`),
}).required()
